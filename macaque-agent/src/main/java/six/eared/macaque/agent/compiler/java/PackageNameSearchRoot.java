...
public class PackageNameSearchRoot implements SearchRoot {
    ...
    public static void loadBootJar(PathJarFile jarFile, String classPrefix, String libPrefix, Set<SearchRoot> classpathRootSet) throws IOException {
        Map<String, PackageNameSearchRoot> packages = new HashMap<>();
        Enumeration<JarEntry> entries = jarFile.entries();
        while (entries.hasMoreElements()) {
            JarEntry entry = entries.nextElement();
    ...
    public List<JavaFileObject> search(String searchPackageName, Set<JavaFileObject.Kind> kinds) {
        if (kinds.contains(JavaFileObject.Kind.CLASS) && searchPackageName.equals(this.packageName)) {
            return classes.stream()
                    .map(item -> new JavaSourceFileObject(item.getUri(), item.getClassName(), JavaFileObject.Kind.CLASS))
