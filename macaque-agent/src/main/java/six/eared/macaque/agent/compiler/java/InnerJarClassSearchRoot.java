...
public class InnerJarClassSearchRoot implements SearchRoot {
    ...
    public class InnerJarIndex {
        ...
        private void loadJar() throws IOException {
            this.tmpFile = new File(FileUtil.getProcessTmpPath(), UUID.randomUUID().toString() + ".jar");
            if (!this.tmpFile.exists()) {
                this.tmpFile.getParentFile().mkdirs();
            }
            FileUtil.writeBytes(this.tmpFile, FileUtil.is2bytes(uri.toURL().openStream()));
